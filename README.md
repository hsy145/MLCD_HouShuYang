...
浏览器访问 `https://hsy-cifar10.minkorx.com/` 即可使用图像分类功能。
...
│   ├── best_rf_cifar10.pkl         # Random Forest 模型 (sklearn)
...
│   └── best_eva_cifar10.pth        # EVA-02 模型权重
├── models/                          # 模型定义
│   ├── Airbench.py                 # CifarNet (94%精度)
│   ├── Airbench96.py               # CifarNet96 (96%精度)
│   ├── airbench96_faster.py        # Airbench96 原始高性能实现
│   ├── CNN.py                      # 基础CNN网络
...
| 模型            | 类型 | 测试集精度    | 说明          |
|---------------|------|----------|-------------|
|Random Forest|传统ML (sklearn) | ~48% | 随机森林 (参数选择) |
| XGBoost       | 传统ML | ~54%     | 梯度提升树       |
...
> **注意**: EVA-02 模型使用了专有的归一化参数 `MEAN=(0.4814, 0.4578, 0.4082)`, `STD=(0.2686, 0.2613, 0.2757)`。
...
高效率 CIFAR-10 分类网络 (CifarNet94)，包含:
- `CifarNet`: 轻量化主网络
- `init_whiten`: 自动白化初始化
- **注意**: 该模型在输出层使用了基于特征维度的缩放因子，以优化 Logits 分布。
